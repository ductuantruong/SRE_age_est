...
for snr in list_snr:
    for data_type in list_data_type:
        os.makedirs(os.path.join(original_data_dir, 'noisy_data_' + args.noise_type, str(snr), data_type), exist_ok=True)

for data_type in list_data_type:
    wav_dir = os.path.join(original_data_dir, data_type)
    def mix_noise(wav_file):
        ...
        for snr in list_snr:
            ...
            path = os.path.join(original_data_dir, 'noisy_data_' + args.noise_type, str(snr), data_type, wav_file)
            torchaudio.save(path, torch.from_numpy(mix_wav).unsqueeze(0), sr)        
    with Pool(processes=6) as pool:
        pool.map(mix_noise, tqdm(os.listdir(wav_dir)))
